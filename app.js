...
// выгрузка по странице "Категория"
...
// при форминировании запроса мы отправляем все на рендер-страницу cat
...
let sql;
sql = "INSERT INTO user_info (username, user_phone, user_email, address) VALUES ('"+data.username+"','"+data.phone+"','"+data.email+"','"+data.address+"')";
con.query(sql, function(error, result2){
  ...
  date=new Date()/1000;
  for (let i = 0; i < result.length; i++){
sql = "INSERT INTO shop_order (date, user_id, id, goods_cost, goods_amount, total) VALUES ("+date +","+userId+","+result[i]['id'] +","+result[i]['cost']+","+data.key[result[i]['id']]+","+data.key[result[i]['id']]*result[i]['cost']+")";
con.query(sql, function(error, result2){
  if(error) throw error;
...
async function sendMail(data, result){
  let res = '<h2>Письмо с интернет-магазина!</h2>';
  let total = 0;
  for (let i=0; i<result.length; i++){
  ...
  let mailOption = {
    from: '<de.qbik2@gmail.com>',
    to: "de.qbik2@gmail.com,"+data.email,
    subject:"SHOP ORDER!!!1!11",
    text:'Hello',
    html: res
  };
